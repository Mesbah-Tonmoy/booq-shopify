...
import { PrimaryActionButton } from '../components/WorkingButtons';
import type { Service } from '@prisma/client';
import type { Route } from './+types/app.service._index';
...
export const action = async ({ request }: Route.ActionArgs) => {
  ...
  if (actionType === 'delete') {
    const serviceId = getInt(formData, 'serviceId');
...
export default function ServiceListPage() {
  ...
  const confirmDelete = useCallback(() => {
    if (serviceToDelete) {
      ...
      formData.append('serviceId', String(serviceToDelete));
        ...
        >
          ...
          <s-table-body>
            {currentPagedServices.length === 0 ? (
              ...
              currentPagedServices.map((service: Service) => (
                <s-table-row key={service.id}>
                  ...
                  <s-table-cell>
                    <s-text color="subdued">No Variations</s-text>
                  </s-table-cell>
