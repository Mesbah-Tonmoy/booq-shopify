...
import type { Route } from './+types/api.services.$id';
...
export const loader = async ({ params }: Route.LoaderArgs) => {
  try {
    ...
    const locationIds = (service.selectedLocations as number[] | null) || [];
    const staffIds = (service.selectedStaff as number[] | null) || [];
    ...
    const response = {
      ...
      // Slot configurations
      slots: service.slots.map((slot: Slots) => ({
        id: slot.id,
