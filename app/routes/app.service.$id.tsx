...
import type { Route } from './+types/app.service.$id';
...
export const loader = async ({ request, params }: Route.LoaderArgs) => {
  ...
  // Merge slot configuration for easier access in components
  const slotConfiguration =
    serviceRecord.slots && serviceRecord.slots.length > 0
      ? serviceRecord.slots[0].slotConfiguration
      : null;

  // Fetch product data from Shopify if product is linked
  let productData: ProductData | null = null;
  if (serviceRecord.shopifyProductId) {
    try {
      const response = await admin.graphql(
        ...
        {
          variables: {
            id: serviceRecord.shopifyProductId,
          },
        }
      );

      const data: ShopifyProductGraphQLResponse = await response.json();
