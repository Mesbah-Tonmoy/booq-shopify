...
import { HOLIDAY_COUNTRIES, getHolidaysForCountry } from '../data/holidayData';

export function useHolidaySettings() {
  const [country, setCountry] = useState(HOLIDAY_COUNTRIES[0]);
  const [countryEnabled, setCountryEnabled] = useState(false);
  const [enabledHolidayIds, setEnabledHolidayIds] = useState([]);
