...
import type {
  ...
  TimeSlot,
  WeeklyHours,
} from '../../stores/workingHoursStore';

const WEEK_DAYS: Day[] = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];
...
export default function CalendarViewModal({
  ...
  const getHoursForCell = (day: number, colIndex: number) => {
  ...
  const handleApply = (slots: TimeSlot[]) => {
    if (!editInfo) return;
